...
def same_loc(board, turn, row, col, lst):
    ...
    same_loc: (listof (listof Str)) Str Nat Nat Any
              -> (anyof Bool (listof (list Nat Nat)))
    Requires: as stated on the top, 0<=row, col<=7
    
    Examples:       
       same_loc(board1,'B',7,0,[]) => False
       same_loc(board1,'B',0,0,[]) => 
               [[0, 3], [2, 0], [4, 0], [7, 0], [2, 2], [5, 5]]
...
def filter_lst(board, turn, row, col, lst, lon):
    ...
    filter_lst: (listof (listof Str)) Str Nat Nat (listof (list Nat Nat)) Any
                -> (anyof Bool (listof (list Nat Nat)))
    Requires: as stated on the top, 0<=row, col<=7
